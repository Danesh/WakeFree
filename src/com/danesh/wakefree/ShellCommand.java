...
/*public class ShellCommand {
	...
	public boolean canSU(boolean force_check) {
		if (can_su == null || force_check) {
			CommandResult r = su.runWaitFor("id",null);
	...
	public class SH {
		...
		private String getAppName(String packageName){
			try {
				return mContext.getPackageManager().getPackageInfo(packageName,PackageManager.GET_META_DATA).applicationInfo.name;
				...
				if (dis.available() > 0) { 
					...
					while(dis.available() > 0){
						...
						if ((pos = out.indexOf("PARTIAL_WAKE_LOCK")) != -1){
							...
							PARTIAL_WAKE_LOCK.add(getAppName(getPackageName(getPid(out,pos))));
		...
		public CommandResult runWaitFor(String s, Context context) {
			if (context!=null)
				mContext = context;
